...
const char * const BITCOIN_CONF_FILENAME = "batamcoin.conf";
const char * const BITCOIN_PID_FILENAME = "batamcoin.pid";
...
#else
    const char* pszModule = "batamcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Batamcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Batamcoin";
#else
    // Unix
    return pathRet / ".batamcoin";
#endif
#endif
}
